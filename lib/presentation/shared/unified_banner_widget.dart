...
class UnifiedBannerWidget extends StatelessWidget {
  ...
  Widget _buildHeader() {
    return Padding(
      ...
      child: section.showArrowIcon
          ...
          : Text(section.title, style: AppTextStyles.typographyH9Medium.copyWith(color: AppColors.textGreyHighest950)),
    );
  }
  ...
  Widget _buildList() {
    return SingleChildScrollView(
      ...
      child: Row(children: List.generate(section.items.length, (index) => _buildItem(index))),
    );
  }
      ...
      child: Stack(
        children: [
          Container(
            height: 180,
            width: 350,
            margin: EdgeInsets.only(left: index == 0 ? 24 : 16, right: index == section.items.length - 1 ? 24 : 0),
            child: ClipRRect(
          ...
          if (_isNotEmpty(item.logo) && item.status == 1) Positioned(top: 16, left: 0, right: 0, child: Center(child: _buildBrandNameChip(item))),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          if (_isNotEmpty(item.logo)) ClipOval(child: AppImage.network(item.logo!, width: 24, height: 24)),
  ...
  Widget _buildBannerBrandLogoName(int index) {
    ...
    if (!_isNotEmpty(item.logo) && !_isNotEmpty(item.coverPhoto) && !_isNotEmpty(item.name)) {
      return const SizedBox.shrink();
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        width: 80,
        margin: EdgeInsets.only(right: index == section.items.length - 1 ? 24 : 16, left: index == 0 ? 24 : 0),
        child: Column(
          children: [
            Stack(
              children: [
                // Hiển thị logo hoặc coverPhoto (ưu tiên logo)
                ClipOval(child: AppImage.network(item.logo ?? item.coverPhoto ?? '', width: 64, height: 64)),
                // Status overlay (unavailable/closed)
                if (item.status != 1)
                  Container(
                    width: 64,
          ...
          children: [
            Stack(
              children: [
                ...
                if (_isNotEmpty(item.logo) && item.status == 1) Positioned(top: 8, left: 8, child: ClipOval(child: AppImage.network(item.logo!, width: 64, height: 64))),
              ],
            ),
            const SizedBox(height: 8),
            Row(
              ...
              children: [
                ...
                if (_hasValidRating(item.rating) && item.rating! > 4.0) Padding(padding: const EdgeInsets.only(left: 8), child: Assets.icons.icVerified.svg()),
              ],
            ),
            // Delivery info - chỉ hiển thị khi có thông tin
            if (_hasValidDeliveryFee(item.deliveryFee) || _isNotEmpty(item.deliveryTime)) _buildDeliveryInfo(item),
          ],
        ),
      ),
    );
  ...
  Widget _buildDeliveryInfo(StoreBanner item) {
    ...
    if (_hasValidDeliveryFee(item.deliveryFee)) {
      infoWidgets.add(Text('\$${item.deliveryFee!.toStringAsFixed(2)} Delivery fee', style: AppTextStyles.typographyH12Regular.copyWith(color: AppColors.textGreyHigh700)));
    }
    ...
    return Row(spacing: 4, children: infoWidgets);
  }
  ...
  Widget _buildBannerSingleImage(int index) {
    ...
    return GestureDetector(
      onTap: item.status == 1 ? section.onTapItem : null,
      child: Padding(
        padding: EdgeInsets.only(right: index == section.items.length - 1 ? 24 : 16, left: index == 0 ? 24 : 0),
        child: SizedBox(
          width: 175,
          height: 220,
          child: Stack(
            children: [
              ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child: AppImage.network(item.coverPhoto ?? item.image ?? '', width: 175, height: 220, fit: BoxFit.cover),
              ),
